...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            if (!Auth::attempt(
                ['kopeg' => $credentials['kopeg'], 'password' => $credentials['password']],
            ...
            // Validation failed (missing / invalid input)
            return back()
                ->withErrors($e->errors())
                ->withInput();
        } catch (AuthenticationException $e) {
            // Wrong kopeg or password
            return back()
                ->withErrors([
                    'kopeg' => 'Kopeg atau password salah.',
                ])
                ->withInput();
        } catch (Throwable $e) {
            // Any unexpected error (DB, session, etc.)
            ...
            return back()
                ->withErrors([
                    'kopeg' => 'Terjadi kesalahan sistem. Silakan coba lagi nanti.',
                ])
                ->withInput();
        }
    }
