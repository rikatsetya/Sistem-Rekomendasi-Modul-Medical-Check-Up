...
@section('content')
    ...
    @else
        @php
            // ambil year yang dipilih dari URL (default year sekarang)
        ...
        <div class="row g-6">
            <div class="col-xl-12 col-sm-12">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <!-- Title -->
                        <div class="col-xl-3 col-lg-3 col-md-2 col-sm-12 align-items-center">
                            <h4 class="card-title">Rekap</h4>
                        </div>

                        <!-- Tahun -->
                        <div class="col-xl-2 col-lg-2 col-md-3 col-sm-12 d-flex align-items-center">
                            <label for="year" class="form-label mb-0 me-2">Tahun</label>
                            @php
                        ...
                        <!-- Import Excel -->
                        <div class="col-xl-2 col-lg-2 col-md-2 col-sm-12 d-flex align-items-end">
                            @include('app.value.import_ajax')
                            <div class="text-md-end text-sm-start w-100">
                                @can('create', App\Models\Value::class)
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#importModal">
                                        <span class="badge text-bg-success">
                                            <i class="icon-base ti tabler-plus icon-xs me-2"></i>Import Excel
                                        </span>
                                    </a>
                                @endcan
                            </div>
                ...
                <a href="{{ route('komponen.darah', ['id' => encrypt($userId) ?? encrypt(Auth::user()->id)]) }}"
                    ...
                    <div class="card h-100 card-hover-shadow">
                        @php
                            ...
                            $currentPlatelet =
                                $trombosit->values->where('tahun', $selectedYear)->first()->nilai ?? null;
                ...
                <a href="{{ route('kimia.darah', ['id' => encrypt($userId) ?? encrypt(Auth::user()->id)]) }}"
                    ...
                    <div class="card h-100 card-hover-shadow">
                        @php
                            ...
                            $creatinineValues = $creatinineData
                                ->pluck('nilai')
                                ->map(fn($val) => round($val, 2))
                                ->toArray();
                            $currentCreatinine =
                                $kreatinin->values->where('tahun', $selectedYear)->first()->nilai ?? null;
            ...
            <div class="col-xl-3 col-sm-6">
                <div class="row g-6">
                    <div class="col-xl-12 col-sm-12">
                        <a
                            href="{{ route('pemeriksaan.diagnostik', ['id' => encrypt($userId) ?? encrypt(Auth::user()->id)]) }}">
                            <div class="card">
                                @php
                        ...
                        <a
                            ...
                            <div class="card">
                                @php
                                    // Ambil kesimpulan untuk tahun terpilih
                                    $spirometriNow =
                                        $spirometri->values->where('tahun', $selectedYear)->first()->nilai ?? null;
                                @endphp
                                <div class="card-header">
                                    <div class="d-flex justify-content-between w-100">
                                        <h6>Spirometri</h6>
...
@push('scripts')
    <script>
        $('#year').on('change', function() {
            ...
            window.location.href = `?year=${selectedYear}&user_id=${selectedUser}`;
        });

        $('#user_id').on('change', function() {
            const selectedUser = $(this).val(); // pakai .val()
            const selectedYear = $('#year').val(); // pakai .val()
            window.location.href = `?year=${selectedYear}&user_id=${selectedUser}`;
        });
    </script>
@endpush
